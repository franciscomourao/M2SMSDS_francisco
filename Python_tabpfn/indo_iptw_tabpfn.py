...
def iptw_tabpfn(tmp_ddata, n_sim_value, n_boot):
    ...
    Y = tmp_ddata['psphinc']
    X = tmp_ddata.drop(columns=['psphinc', 'outcome', 'n_sim'])

    model = TabPFNClassifier()
    model.fit(X, Y)

    tmp_ddata['ps'] = model.predict_proba(X)[:, 1]
    tmp_ddata['poids'] = np.where(tmp_ddata['psphinc'] == 1, 1 / tmp_ddata['ps'],
                1 / (1 - tmp_ddata['ps']))
    
    logreg = LogisticRegression()
    logreg.fit(X=tmp_ddata['psphinc'].values.reshape(-1, 1),
                y=tmp_ddata['outcome'],
    ...
    for _ in range(n_boot):
        ...
        if df_bootstrap_sample['outcome'].nunique() == 1:
            ...
            Y = df_bootstrap_sample['psphinc']
            X = df_bootstrap_sample.drop(columns=['psphinc', 'outcome', 'n_sim'])
            ...
            logreg = LogisticRegression()
            logreg.fit(
                X=df_bootstrap_sample['psphinc'].values.reshape(-1, 1),
